...
#  Description: A collection of various recursive functions.
...
def groupSumClump(start, nums, target):
	if (start >= len(nums)):
		...
		while (foundAll == False):
			if (indexLeftOff + 1 < len(nums)) and (nums[indexLeftOff] == nums[indexLeftOff + 1]):
				currentSum += nums[start + 1]
				indexLeftOff += 1
			else:
				foundAll = True
		return groupSumClump (indexLeftOff + 1, nums, target - currentSum) or groupSumClump (indexLeftOff + 1, nums, target)
	else:
...
def splitArray(nums):
	list1 = []
	list2 = []
	return splitArrayHelper (0, nums, list1, list2)


def splitArrayHelper(start, numsList, list1, list2):
	if (start >= len (numsList)):
		return (sum(list1) == sum(list2))
	else:
		return splitArrayHelper (start + 1, numsList, list1 + [numsList[start]], list2) or splitArrayHelper (start + 1, numsList, list1, list2 + [numsList[start]])
...
def splitOdd10(nums):
	list1 = []
	list2 = []
	return splitOdd10Helper (0, nums, list1, list2)

  
def splitOdd10Helper(start, numsList, list1, list2):
	if (start >= len (numsList)):
		return (sum(list1) % 10 == 0 and sum(list2) % 2 == 1) or (sum(list1) % 2 == 1 and sum(list2) % 10 == 0)
	else:
		return (splitOdd10Helper (start + 1, numsList, list1 + [numsList[start]], list2)
			or splitOdd10Helper (start + 1, numsList, list1, list2 + [numsList[start]]))
...
def split53(nums):
	...
	distribAll = False
	index = 0
	while distribAll == False:
		if (index > len(nums)-1) or (nums == []) or (not nums):
			distribAll = True
		elif (nums[index] % 5 == 0):
	...
	return split53Helper (0, nums, list1, list2, emptyList1, emptyList2)


def split53Helper(start, numsList, listFives, listThrees, list1, list2):
	if (start >= len(numsList)):
		newList1Fives = [] + list1 + listFives
		newList1Threes = [] + list1 + listThrees
		newList2Fives = [] + list2 + listFives
		newList2Threes = [] + list2 + listThrees
		return (sum(newList1Fives) == sum(newList2Threes)) or (sum(newList1Threes) == sum(newList2Fives))
	else:
		return (split53Helper (start + 1, numsList, listFives, listThrees, list1 + [numsList[start]], list2)
			or split53Helper (start + 1, numsList, listFives, listThrees, list1, list2 + [numsList[start]]))
